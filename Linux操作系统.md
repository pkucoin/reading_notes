...
## 进程的创建
创建进程的过程实际上就是为子进程创建一个进程描述符以及用户内存空间的过程。其中进程描述符的内容大多根据父进程的内容进行复制，用户内存空间的大部分也共享或复制自父进程。下面以fork()调用为主介绍进程创建的细节，然后介绍fork(), vfork(), clone(), exec()之间的异同。
...
fork的名字形象地表明了其行为：子进程和父进程将在fork()之后“分叉”，分别在fork()的返回处继续执行。fork()执行成功返回时，在父进程中的返回值是子进程的pid，而在子进程中的返回值则是0。执行失败时将返回-1，可能的原因是已创建进程数量超过了系统限制。在Linux上，fork()返回后首先调度执行的是父进程还是子进程取决于/rpoc/sys/kernel/sched_child_runs_first，从2.6.32开始该值为0，意味着父进程将首先被调度执行。但不应该对这一顺序做任何假定，如果需要保证特定的执行顺序，应该明确使用某种同步技术。

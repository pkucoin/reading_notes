...
# 参考文献
阅读书籍以《Linux Programming Interface》为主，APUE和《现代操作系统（第3版）》为辅，同时参考了若干blog博文和笔记。相比于经典的APUE，LPI的突出优点有两点：
...
具体来说，虚拟内存将内存分割为固定大小的页（常见的大小是4096字节），物理内存被划分为一系列该大小的页帧。物理内存在虚拟内存机制中可以被看做一个cache。每个进程最近使用到的部分页驻留在物理内存中，称为驻留集resident set；未使用到的部分保存在磁盘的swap area中。进程运行时使用虚拟内存地址，CPU的MMU会通过查询内存中的页表得到对应的物理地址。如果进程要访问的页不在物理内存中，则发生一次缺页中断page fault，内核挂起当前进程，将该页从磁盘中载入物理内存。（这只是page fault中的一种，另两种为：a. 页未分配：segment fault；b. 页已分配但权限错误：protection exception）

按照上述思路，在32位系统中需要1M个页帧，如果每个页帧记录一个4字节的地址，则页表需要4MB的内存，这4MB的内存占用是常驻的，对于64位系统更是天文数字，故这种方案是不可行的。因此Linux引入了多级页表：原本地址中前20位为页号，后12位为页内偏移。现在将前20位中的前10位作为页目录，后10位作为页目录中的页号。当一个目录中全部为未分配内存时，该目录就不需要一个页表。

## 内存分配
### brk和sbrk
堆的起点也就是BSS段的末尾，始终保持不变。改变堆的大小也就是改变高地址边界program break的位置。系统调用brk(void*)直接将program break设置为参数地址，而sbrk(int)将program break增加参数指定的大小。

### malloc与free
C库函数malloc(size_t)在堆上分配参数字节大小的内存，返回指向分配内存起始位置的指针（总是采用字节对齐的方式），当无法分配内存时返回NULL。

free(void*)则释放指针参数指向的内存，该指针参数应该是由堆内存分配函数返回的地址。free()通常并不降低program break的位置。free(NULL)什么都不做（并不会报错）。free（ptr）调用后再使用ptr将导致不可知的后果。虽然在进程终止时其占用的所有内存均会被释放，但是仍然应该显式调用free()。
...
## 进程内存布局
![进程内存布局](https://github.com/pkucoin/reading_notes/blob/master/pics/%E8%BF%9B%E7%A8%8B%E5%86%85%E5%AD%98%E5%B8%83%E5%B1%80.JPG "进程内存布局")

进程的内存空间由若干个段组成，按照低地址到高地址的顺序依次为：
- 文本段。包含程序机器语言指令。只读意外修改。多进程运行同一程序，文本段可共享。
- 初始化数据段。包含显式初始化的全局变量和静态变量。程序加载时进程问可执行文件中读取这些值。
- BSS（block started by symbol）段。包含未显式初始化的全局变量和静态变量，初始化为0。程序并没有必要为这些变量占用磁盘空间，当程序加载时进程再为这些变量分配内存空间并初始化为0。
- 堆。运行时进行的动态内存分配的区域。堆的低地址始于BSS段末尾，高地址的内存边界成为program break，堆的大小也就是改变program break的位置。
- 共享内存和共享库。
- 栈。动态增长和收缩的段，由stack frames组成。每一个frame包含了当前调用的函数的局部变量、实参和返回值，并保存了寄存器值防止被调用函数污染。

## 函数调用与栈、栈帧

函数调用约定（calling convention）规定了函数调用过程中的参数入栈顺序、调用前栈准备、调用后栈清理等。在一次函数调用过程中，寄存器ebp(base pointer)始终指向当前栈帧的底部，在该函数运行过程中保持不变，局部变量和函数参数以ebp为基础加上偏移量来访问。寄存器esp(stack pointer)指向当前栈帧的顶部，随程序运行变化。下面以cdecl为例，讲述参数从右到左压栈、由调用者负责清理参数的函数调用约定：

## 进程创建
创建进程的过程实际上就是为子进程创建一个进程描述符以及用户内存空间的过程。其中进程描述符的内容大多根据父进程的内容进行复制，用户内存空间的大部分也共享或复制自父进程。下面以fork()调用为主介绍进程创建的细节，然后介绍fork(), vfork(), exec(), clone()之间的异同。
...
## 进程退出
### exit()与_exit()
系统调用void \_exit(int status)使进程终止，status为0表示正常退出，非0值（可取0~255，但大于128会引起混乱）表示异常退出，父进程通过wait()调用获取到的值即为status。

通常并不直接调用\_exit()，而是调用c库函数exit()。exit()会在调用\_exit()之前调用退出处理程序并刷新stdio流缓冲区。从main函数返回或者执行到main函数结束相当于执行了exit(返回值或0)。

### atexit与on_exit
使用atexit可以注册（至少32个）无参数也无返回值的函数，使得这些函数在exit()时被调用，调用顺序与注册顺序相反。直接使用_exit()或使用信号结束进程不会调用atexit注册的这些函数。fork()创建的子进程会继承父进程注册的退出处理函数。

使用atexit注册的函数既无法得知exit的status，也无法传递其他参数。glibc提供了一个（非标准的）替代方法：int on_exit(void (\*func)(int, void\*), void \*arg)。它与exit注册的函数位于同一列表中。
...
fd的数值只是打开该fd的进程中文件描述符表的序号。因此，不同进程中相同的fd可以指向不同的文件，而不同的fd也可以指向同一个文件，它们将共享该文件的偏移量和打开文件状态标志，但fd的close-on-exec标志仍为进程所私有。在shell中0, 1, 2这三个fd始终是打开的，依次用于标准输入、标准输出和标准错误，所以程序也继承了这三个fd。
...
文件偏移量是打开文件句柄（而不是文件描述符）的属性。如果多个线程对同一文件描述符或者指向同一打开文件句柄的不同文件描述符同时进行操作，文件偏移量会在这些线程间共享，改变也会对这些线程可见，这会造成race condition。pread()和pwrite()调用可以进行指定位置的文件IO，这一过程相当于先lseek到指定位置再read/write，不同的是该过程是原子的，并且不会改变文件的当前偏移量。
